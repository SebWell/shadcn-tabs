...
export default {
  ...
  setup(props, { emit }) {
    const activeTab = ref(props.modelValue !== null ? props.modelValue : props.content.defaultTab)

    const tabsClasses = computed(() => cn(
      "w-full",
      {
        "flex": props.content.orientation === 'vertical',
        "flex-col": props.content.orientation === 'horizontal'
      },
    ...
    const tabsListClasses = computed(() => cn(
      ...
      {
        "h-10": props.content.size === 'default',
        "h-8": props.content.size === 'sm',
        "h-12": props.content.size === 'lg',
        "flex-col h-auto w-auto": props.content.orientation === 'vertical'
      }
    ))
    ...
    const getTabTriggerClasses = (index) => {
      return cn(
        "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
        {
          ...
          "gap-2": props.content.tabs[index]?.icon
        }
      )
    }
    ...
    const getTabContentClasses = (index) => {
      return cn(
        ...
        {
          "ml-2 mt-0": props.content.orientation === 'vertical'
        }
      )
    }
    ...
    const setActiveTab = (index) => {
      if (index !== activeTab.value) {
        ...
        emit('change', { activeTab: index, tab: props.content.tabs[index] })
    ...
    const handleKeyDown = (event, currentIndex) => {
      const tabCount = props.content.tabs.length
